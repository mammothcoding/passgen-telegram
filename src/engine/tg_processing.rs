pub mod tg_processing {
    use crate::engine::db_processing::db_processing::{
        ...
        get_user_dialog_context, get_web_state_token, increase_user_gen_count, set_last_mess_id,
    ...
    async fn web_stat_links_menu(
        ...
        create_token(chat_id, bot_id).await;
        let get_token_result = get_web_state_token(chat_id, bot_id).await;
            ...
            Some(token) => {
                ...
                inline_btns.push(Vec::from([
                    InlineKeyboardButton::callback(
                        format!("{} 🔁", user_lang_map["menu_stat_btn_regen"]),
                        "menu_stat_btn_regen",
                    ),
                    InlineKeyboardButton::callback(
                            ...
                            match text.parse::<String>() {
                                Ok(c_chset) => {
                                    if c_chset.len() < 1000 {
                                        ...
                                        let set_rules_res =
                                            update_rules(chat_id_i64, bot_id_i64, rules).await;

                                        if set_rules_res {
                                            set_user_dialog_context(
                                                chat_id_i64,
                                                bot_id_i64,
                                                "NULL",
                                            )
                        ...
                        Some(context) if context == "pwd_len".to_string() => {
                            match text.parse::<u64>() {
                                Ok(mut pwd_len) => {
                                    ...
                                    if set_rules_res {
                                        set_user_dialog_context(chat_id_i64, bot_id_i64, "NULL")
                                            .await;
                                        remove_prev_mess(
                                            &bot,
                                            chat_id,
    ...
    pub async fn callback_handler(
        ...
        match action {
                    ...
                    || act == "enab_strong_usab".to_string() =>
            {
                ...
                if set_rules_res {
                    remove_prev_mess(&bot, chat_id, chat_id_i64, bot_id_i64, "last_gen_mess_id")
                        .await;
                    remove_prev_mess(&bot, chat_id, chat_id_i64, bot_id_i64, "last_menu_mess_id")
                        .await;
                    gen_and_send_main_menu(
                        &env_data,
                        &bot,
            ...
            Some(act) if act == "generate".to_string() => {
                ...
                increase_user_gen_count(chat_id_i64, bot_id_i64, rules.pwd_quantity).await;
                ...
                if act == "statistics".to_string() || act == "menu_stat_btn_regen".to_string() =>
            {
                ...
                {
                    let web_stat_links_menu_result =
                        web_stat_links_menu(&env_data, user_lang_map, chat_id_i64, bot_id_i64)
                            .await;
